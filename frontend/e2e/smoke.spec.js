...
// `username` defaults to the email, but real SRP sign-ins store the pool's
// internal id there — pass one to reproduce that shape.
async function signIn(page, { username = EMAIL } = {}) {
...
test('own grants read “by you” even when Cognito stores an internal username', async ({ page }) => {
  // After an SRP sign-in, getUsername() returns the pool's internal id, not
  // the email — attribution must compare against the token's email claim or
  // the viewer's own refetched grants degrade from "by you" to their email.
  await signIn(page, { username: 'e2e-internal-uuid-1234' })
  await mockApi(page)
  await page.route(`${API}/classrooms/c1/students/s1`, (route) =>
    route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({
        ...students[0],
        history: [{ studentId: 's1', delta: 2, reason: 'Kindness', timestamp: '2026-07-01T10:00:00.000Z', yearId: 'y1', grantedBy: EMAIL }],
        historyCursor: null,
      }),
    }))
  await page.goto('/#/students/s1')
